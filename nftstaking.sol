...
//pause function
//In the stake function, the farmRate mapping is not correctly updated for the devWallet.
//In the withdraw function, the farmRate mapping is not correctly updated for the devWallet.
...
function farmRate(uint tokenId) external view returns(uint);
 function setIsStaked(uint _tokenId, bool _currentState) external;
...
contract ERC721Staking is ReentrancyGuard, AccessControl {
...
event Withdraw(address indexed staker, uint256 tokenId);
    ...
    constructor(IERC721 _nftCollection, address _rewardsToken) {
